...
import { getAllDiagrams, deleteDiagram, getApiKey, getSettings } from '@/lib/storage'
...
export default function HomePage() {
  ...
  const handleImageReady = async (base64: string, mediaType: string, fileName?: string) => {
    ...
    if (!settings.apiKey) {
      setError('Add an AI API key in Settings first.')
      return
    }
    setExtracting(true)
    setError(null)
    try {
      const providerLabels = { gemini: 'Gemini', anthropic: 'Claude', openai: 'GPT-4o', groq: 'Llama' }
      setExtractProgress(`Sending image to ${providerLabels[settings.aiProvider]}…`)
      const extracted = await extractDiagramFromImage(base64, mediaType, settings.apiKey, settings.aiProvider)
    ...
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
      ...
      {!hasApiKey && !showUpload && diagrams.length === 0 && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-xl border border-amber-800/40 bg-amber-950/30 text-amber-300 text-sm">
          ...
          <p>
            You need an AI API key to extract diagrams.{' '}
